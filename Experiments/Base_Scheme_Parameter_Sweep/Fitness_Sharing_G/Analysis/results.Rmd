...
over_time_df <- read.csv(paste(DATA_DIR,'OVER-TIME/gfs.csv', sep = "", collapse = NULL), header = TRUE, stringsAsFactors = FALSE)
over_time_df$Sigma <- factor(over_time_df$Sigma, levels = FS_LIST)

best_df <- read.csv(paste(DATA_DIR,'BEST/gfs.csv', sep = "", collapse = NULL), header = TRUE, stringsAsFactors = FALSE)
best_df$Sigma <- factor(best_df$Sigma, levels = FS_LIST)

sati_df <- read.csv(paste(DATA_DIR,'SOL-FND/gfs.csv', sep = "", collapse = NULL), header = TRUE, stringsAsFactors = FALSE)
...
```{r gfs-exp-per-ot, warning=FALSE, out.width="100%"}
...
```{r gfs-exp-per-bst, out.width="100%"}
...
```{r gfs-ord-per-ot, warning=FALSE, out.width="100%"}
...
```{r gfs-ord-per-bst, out.width="100%"}
...
```{r gfs-con-act-ot, warning=FALSE, out.width="100%"}
...
```{r gfs-con-sat-ot, warning=FALSE, out.width="100%"}
lines = filter(over_time_df, acro == 'con')  %>%
  group_by(Sigma, gen) %>%
  dplyr::summarise(
    min = min(pop_uni_obj),
...
```{r gfs-mpe-act-ot, warning=FALSE, out.width="100%"}
...
```{r gfs-mpe-act-kru-end}
kruskal.test(uni_str_pos ~ Sigma, data = act_coverage)
```

Results for post-hoc Wilcoxon rank-sum test with a Bonferroni correction.

```{r gfs-mpe-act-wil-end}
...
```{r gfs-mpe-per-ot, warning=FALSE, out.width="100%"}
lines = filter(over_time_df, acro == 'mpe') %>%
  group_by(Sigma, gen) %>%
  dplyr::summarise(
    min = min(pop_fit_max) / DIMENSIONALITY,
...
```{r gfs-mpe-per-bst, out.width="100%"}
