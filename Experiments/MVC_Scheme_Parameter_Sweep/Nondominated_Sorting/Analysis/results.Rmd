...
over_time_df <- read.csv(paste(DATA_DIR,'OVER-TIME-MVC/nds.csv', sep = "", collapse = NULL), header = TRUE, stringsAsFactors = FALSE)
over_time_df$Sigma <- factor(over_time_df$Sigma, levels = ND_LIST)

best_df <- read.csv(paste(DATA_DIR,'BEST-MVC/nds.csv', sep = "", collapse = NULL), header = TRUE, stringsAsFactors = FALSE)
...
```{r nds-exp-per-ot, out.width="100%"}
...
```{r nds-exp-val-bst, out.width="100%"}
...
```{r nds-ord-per-ot, out.width="100%"}
...
```{r nds-ord-val-bst, out.width="100%"}
...
```{r nds-con-act-ot, out.width="100%"}
...
```{r nds-con-sat-ot, out.width="100%"}
lines = filter(over_time_df, acro == 'con')  %>%
  group_by(Sigma, gen) %>%
  dplyr::summarise(
    min = min(pop_uni_obj),
...
```{r nds-con-val-bst, out.width="100%"}
...
```{r nds-mpe-act-ot, out.width="100%"}
...
```{r nds-mpe-per-ot, out.width="100%"}
lines = filter(over_time_df, acro == 'mpe') %>%
  group_by(Sigma, gen) %>%
  dplyr::summarise(
    min = min(pop_fit_max) / DIMENSIONALITY,
...
```{r nds-mpe-val-bst, out.width="100%"}
